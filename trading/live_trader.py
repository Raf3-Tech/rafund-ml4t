...
def _place_market_order(exchange, symbol: str, side: str, qty: float) -> Optional[dict]:
    """Place a market order; returns the order dict or None on failure."""
    try:
        order = exchange.create_market_order(symbol, side, qty)
        logger.info("live_trader: %s %s qty=%.6f  id=%s", side.upper(), symbol, qty, order.get("id"))
        return order
    except Exception as exc:
        logger.error("live_trader: order failed — %s", exc)
        return None
...
# ── live-specific close / open (routes through CCXT) ─────────────────────────


def _live_close(pos: PositionState, exchange, current_price: float) -> Tuple[float, Optional[str]]:
    """Place a close order; return (pnl, exchange_order_id)."""
    ccxt_side = "sell" if pos.side == "LONG" else "buy"
    order = _place_market_order(exchange, pos.symbol, ccxt_side, abs(pos.qty))
    fill = _fill_price(order, current_price) if order else current_price
    oid = order["id"] if order else None
    pnl = _close(pos, fill)
    commission = abs(pos.qty * fill) * _COMMISSION_PCT  # already deducted by _close via equity
    log_order(db_ref := None, pos, "CLOSE", fill, pnl=pnl, commission=commission,
              order_type="live", exchange_order_id=oid)
    return pnl, oid


def _live_open(pos: PositionState, target: str, exchange, current_price: float, cfg) -> Optional[str]:
    """Place an open order; return exchange_order_id or None on failure."""
    notional = min(pos.equity * cfg.leg_allocation_pct, _max_notional())
    qty = notional / current_price
    ccxt_side = "buy" if target == "LONG" else "sell"
    order = _place_market_order(exchange, pos.symbol, ccxt_side, qty)
    if order is None:
        return None
    fill = _fill_price(order, current_price)
    oid = order["id"]
    _open(pos, target, fill, notional)
    log_order(None, pos, "OPEN", fill, commission=notional * _COMMISSION_PCT,
              order_type="live", exchange_order_id=oid)
    return oid
...
def run_live_cycle(db) -> bool:
    ...
    cfg = get_settings()
    exchange_name = os.environ.get("LIVE_EXCHANGE", "binance").lower()
    ...
    if result is None:
        logger.info("live_trader: no accepted strategy yet — skipping")
        return True
    ...
    if df is None or len(df) < strategy.get_min_bars(params):
        logger.warning("live_trader: insufficient data for %s", symbol)
        return True

    signal = _last_signal(df, strategy_name, params)
    price = float(df["close"].iloc[-1])

    pos = load_position(db, LIVE_RUN_ID, initial_capital=cfg.account_size,
        ...
        if pos.is_flat:
            if not pos.daily_halt:
                ...
                if shadow:
                    ...
                    log_order(db, pos, "OPEN", price, order_type="shadow")
                else:
                    ccxt_side = "buy" if target == "LONG" else "sell"
                    order = _place_market_order(exchange, symbol, ccxt_side, qty)
                    if order:
                        fill = _fill_price(order, price)
                        _open(pos, target, fill, notional)
                        log_order(db, pos, "OPEN", fill, commission=notional * _COMMISSION_PCT,
                                  order_type="live", exchange_order_id=order["id"])
            ...
            if shadow:
                ...
                log_order(db, pos, "CLOSE", price, pnl=pnl, order_type="shadow")
                if not pos.daily_halt:
                    notional = min(pos.equity * cfg.leg_allocation_pct, _max_notional())
                    _open(pos, target, price, notional)
                    log_order(db, pos, "OPEN", price, order_type="shadow")
                ...
                ccxt_side = "sell" if pos.side == "LONG" else "buy"
                order = _place_market_order(exchange, symbol, ccxt_side, abs(pos.qty))
                fill = _fill_price(order, price) if order else price
                oid = order["id"] if order else None
                pnl = _close(pos, fill)
                log_order(db, pos, "CLOSE", fill, pnl=pnl, order_type="live", exchange_order_id=oid)
                ...
                if not pos.daily_halt:
                    ...
                    order2 = _place_market_order(exchange, symbol, ccxt_side2, qty)
                    if order2:
                        ...
                        log_order(db, pos, "OPEN", fill2, commission=notional * _COMMISSION_PCT,
                                  order_type="live", exchange_order_id=order2["id"])
    else:
        if not pos.is_flat:
            if shadow:
