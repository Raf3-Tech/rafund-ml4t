"""
HTX (formerly Huobi) data collector using CCXT.

Mirrors BinanceCollector exactly — same validation, same retry policy, same
CollectionResult contract. HTX spot trades USDT pairs (BTC/USDT, ETH/USDT …)
...
# HTX spot pairs — USDT, same quote currency as Binance
DEFAULT_HTX_SYMBOLS = [
    'BTC/USDT',
    'ETH/USDT',
    'SOL/USDT',
...
class HTXCollector:
    """Fetches market data from HTX exchange with retries and validation."""

    def __init__(self, rate_limit_ms: int = 500):
        self.rate_limit_ms = rate_limit_ms
        self.client: Optional[ccxt.htx] = None
    ...
    def _initialize_client(self) -> None:
        try:
            ...
            logger.error('Failed to initialize HTX client', error=str(exc))
            raise
    ...
    def fetch_ohlcv_safe(
        ...
        try:
            ...
            logger.error(
                'HTX request failed after retries',
                symbol=symbol,
                timeframe=timeframe,
    ...
    def collect_symbol(
        ...
        # HTX returns up to 720 daily candles per request (same cap as Kraken)
        batch_limit = 720
        ...
        logger.info(
            'Completed HTX symbol collection',
            symbol=symbol,
            timeframe=timeframe,
            records_fetched=records_fetched,
            records_inserted=records_inserted,
